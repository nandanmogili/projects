...
def api():
    ...
    most_similar_entry = search(df, message, n=1) # Variable for the most similar entry in csv
    similarity_score = most_similar_entry.iloc[0]['similarities'] # Variable for entry's similarity 
    response = "" 

    if similarity_score < .80: # Determines if entry is irrelevant
        response = "I am unable to generate a proper response at this time."
    else:
        response = most_similar_entry.iloc[0]['Answer'] # if relevant, return entry's answer 
